...
(* let rec search_for_muls str search_index curr_sum enable = *)
(*   if enable then *)
(*     try *)
(*       let start_pos = Str.search_forward mul_regex str search_index in *)
(*       let matched_str = Str.matched_string str in *)
(*       let end_pos = Str.match_end () in *)
(*       try *)
(*         let dont_start_pos = Str.search_forward dont_regex str search_index in *)
(*         let dont_end_pos = Str.match_end () in *)
(*         if dont_start_pos < start_pos then *)
(*           search_for_muls str dont_end_pos curr_sum false *)
(*         else *)
(*           let _ = Str.search_forward num_regex matched_str 1 in *)
(*           let left_num = int_of_string (Str.matched_string matched_str) in *)
(*           let _ = Str.search_forward num_regex matched_str (Str.match_end ()) in *)
(*           let right_num = int_of_string (Str.matched_string matched_str) in *)
(*           search_for_muls str end_pos (curr_sum + (left_num * right_num)) enable *)
(*       with Not_found -> *)
(*         let _ = Str.search_forward num_regex matched_str 1 in *)
(*         let left_num = int_of_string (Str.matched_string matched_str) in *)
(*         let _ = Str.search_forward num_regex matched_str (Str.match_end ()) in *)
(*         let right_num = int_of_string (Str.matched_string matched_str) in *)
(*         search_for_muls str end_pos (curr_sum + (left_num * right_num)) enable *)
(*     with Not_found -> (curr_sum, enable) *)
(*   else *)
(*     try *)
(*       let _ = Str.search_forward do_regex str search_index in *)
(*       let end_pos = Str.match_end () in *)
(*       search_for_muls str end_pos curr_sum true *)
(*     with Not_found -> (curr_sum, enable) *)
(* let rec input_helper curr_sum curr_enable = *)
(*   try *)
(*     let input_string = input_line input in *)
(*     let sum, enable = search_for_muls input_string 0 curr_sum curr_enable in *)
(*     input_helper sum enable *)
(*   with End_of_file -> curr_sum *)
(* let () = Printf.printf "%d\n" (input_helper 0 true) *)

(* Terse version of Part 2 *)
