...
let inp_lists =
  let rec input_helper acc =
    try
      input_line input |> String.split_on_char ' ' |> function
      | test_val :: test_list ->
          let test_val' =
            String.sub test_val 0 (String.length test_val - 1) |> int_of_string
          in
          (test_val' :: List.map int_of_string test_list) :: acc |> input_helper
